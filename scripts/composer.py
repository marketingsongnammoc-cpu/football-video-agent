...
def _render_scene(frame_path: Path, audio_path: Path,
    ...
    """
    Render 1 scene: static frame PNG + audio → MP4.
    Motion zoom nhẹ 1.0→1.03 (hoặc ngược) bằng ffmpeg zoompan.
    Không PIL per-frame → nhanh (~5s thay vì 3 phút).
    """
    audio_dur = _get_audio_duration(audio_path)
    total_dur = audio_dur + SILENCE_AFTER
    n_frames  = int(total_dur * FPS)

    # Zoom nhẹ: 1.0→1.03 hoặc 1.03→1.0
    # Ở mức 3%, integer rounding của zoompan < 0.3px → không thấy được
    if motion in ("zoom_in", "pan_right"):
        z_expr = f"1+0.03*on/{n_frames}"       # 1.0 → 1.03
    else:
        z_expr = f"1.03-0.03*on/{n_frames}"    # 1.03 → 1.0

    # Fade in 0.3s + fade out 0.3s
    fade_frames = int(0.3 * FPS)
    vf = (
        f"zoompan=z='{z_expr}':d={n_frames}:s={CANVAS_W}x{CANVAS_H}:fps={FPS},"
        f"fade=t=in:st=0:d=0.3,"
        f"fade=t=out:st={total_dur - 0.3:.3f}:d=0.3"
    )
...
def compose_video(scenes: list[dict], video_folder: Path, output_path: Path,
    ...
    for idx, scene in enumerate(scenes):
        ...
        # Frame: overlay_XX.png là composite cuối (template + ảnh + text)
