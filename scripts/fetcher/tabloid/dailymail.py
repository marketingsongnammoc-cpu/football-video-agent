...
class DailyMailAdapter(BaseAdapter):
    def list_latest(self, limit: int = 20) -> list[str]:
        html = self._get(self.config.homepage)
        pattern = re.compile(r"href=\"(https?://(?:www\.)?dailymail.co.uk/[^\"#?]+)\"", re.IGNORECASE)
        urls, seen = [], set()
    ...
    def fetch_article(self, url: str) -> Article:
        ...
        return Article(
            ...
            source_name="Daily Mail", language="en",
        )
